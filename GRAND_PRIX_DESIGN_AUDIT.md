...
## Item lane and depth-response validation

The v31 deterministic race view was checked in a live browser at the standard 1440×900 desktop frame. The first view showed the compact ITEM slot beside the existing Dash core, a glowing projected Starbox placed on the road ahead, repeated roadside depth markers, a larger high-speed road field, and the player kart retained inside the lower central safe zone. The Starburst scene then showed the item slot transition to `LOCK #1`, the projected target kart, multiple depth-scaled Starboxes, the speed HUD at about 114 km/h, and the curve look-ahead without a HUD collision. The defensive road-space rule remains intact: the new objects provide a line choice but do not create a wall across the track. A follow-up view positioned the enlarged star-comet at the centre of its curved flight line; it is now visibly distinct from the stationary Starboxes and communicates a forward attack without obscuring either kart.
