...
import { chooseGrandPrixGate, createGrandPrix, finishGrandPrix, startGrandPrix, setGrandPrixBrake, setGrandPrixDrift, setGrandPrixThrottle, steerGrandPrix, takeGrandPrixCorrection, tickGrandPrix, useGrandPrixItem, useGrandPrixJump, useGrandPrixSkill } from "./grand-prix-model.mjs?v=20260814-grandprix-v35";
import { renderGrandPrixScene, updateGrandPrixScene } from "./grand-prix-scene.mjs?v=20260814-grandprix-v35";
...
dom.stage.addEventListener("click", event => {
  if (state.mode === "grandprix" && state.grandPrix && state.phase === "playing") {
    ...
    const item = event.target.closest("[data-gp-item]");
    ...
    if (item) { playGrandPrixEvents(useGrandPrixItem(state.grandPrix)); refreshGrandPrixScene(); return; }
...
document.addEventListener("keydown", event => {
  ...
  if (state.phase === "playing" && state.mode === "grandprix" && state.grandPrix) {
    ...
    if (event.key.toLowerCase() === "e") {
      event.preventDefault();
      if (!event.repeat) playGrandPrixEvents(useGrandPrixItem(state.grandPrix));
      return;
    }
...
function playGrandPrixEvents(events = []) {
  ...
  if (types.has("item-box")) { audio.playSfx("pop"); return; }
...
if (grandPrixDemo === "grandprix") {
  window.setTimeout(() => {
    ...
    if (grandPrixDemoScene === "starburst") {
      state.grandPrix.progress = 520;
      state.grandPrix.drive.speed = 116;
      state.grandPrix.drive.heldItem = "starburst";
      const target = state.grandPrix.racers[0];
      target.distance = state.grandPrix.progress + 92;
      target.progress = target.distance;
      target.lane = 0.25;
      useGrandPrixItem(state.grandPrix);
      state.grandPrix.starburst.durationMs = 5000;
      state.grandPrix.starburst.elapsedMs = 2300;
      refreshGrandPrixScene();
      return;
    }
    setGrandPrixThrottle(state.grandPrix, true);
