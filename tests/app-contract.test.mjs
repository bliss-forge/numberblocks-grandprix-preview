...
test("1~5 모바일 보정 스타일은 기본 스타일 뒤에 로드된다", () => {
  const baseIndex = html.indexOf(
    'href="styles.css?v=20260814-grandprix-v35"'
  );
...
test("스타일 시트는 최신 캐시 주소를 달고 나간다", () => {
  ...
  assert.match(
    html,
    /<link rel="stylesheet" href="styles\.css\?v=20260814-grandprix-v35">/
  );
  assert.match(
    html,
...
test("grand prix Starbox item slot and starburst input stay distinct from Dash", () => {
  ...
  assert.match(grandPrixScene, /MAX_CANVAS_PIXELS = 1250000/);
  assert.match(grandPrixScene, /projectionCaches/);
  assert.match(grandPrixScene, /drawStarbox/);
  assert.match(grandPrixScene, /drawStarburst/);
  assert.match(grandPrixScene, /assets\/grand-prix\/karts\/four\.png/);
  assert.match(app, /useGrandPrixItem/);
  assert.match(app, /event\.key\.toLowerCase\(\) === "e"/);
  assert.match(app, /types\.has\("star-dash-pass"\) \|\| types\.has\("slingshot"\)/);
  assert.match(app, /grandPrixDemoScene === "corner"/);
