...
[![테스트](https://img.shields.io/badge/테스트-327개-7BC67B?style=flat-square)](#테스트)
...
flowchart LR
    ...
    home --> subway[지하철 여행<br/>10개 노선·67개 역]
...
| | |
| --- | --- |
| 노선 | 10개 (실제 1~9호선 + 이 게임에만 있는 10호선) |
| 역 | 67개, 이 중 환승역 32개 |
...
창밖 풍경은 노선마다 다릅니다 — 1·9호선은 지상, 2·3·4·10호선은 한강을 건너고, 5~8호선은 지하를 달립니다.
내릴 때는 승강장 틈을 폴짝 뛰어넘는 짧은 놀이가 있고, 네 번째까지 놓치면 그냥 건너가게 해 줍니다.

#### 10호선 — 서울에는 없는 노선

서울에 10호선은 없습니다. 도하가 좋아해서 이 게임에만 놓은 청록색 노선이고,
재미있는 곳들을 한 줄로 꿰어 **갈아타지 않고 다섯 군데**에 닿습니다.

```
월드컵경기장 ─ 홍대입구 ─ 경복궁 ─ 명동 ─ 도하 ─ 약수 ─ 왕십리 ─ 어린이대공원 ─ 잠실
```

갈아타지 않고 닿는 곳은 하늘공원(월드컵경기장)·경복궁·남산타워(명동)·어린이대공원·놀이공원(잠실)입니다.
가운데 **도하역**은 이 노선에만 있는 역이라 갈아탈 일이 없습니다.
호선은 숫자키 하나로 고르는데 10은 두 자리라 <kbd>0</kbd> 키를 씁니다 —
목적지 열 번째도 이미 <kbd>0</kbd>이라 아이가 배운 규칙 그대로이고,
10호선 버튼에만 눌러야 할 키를 따로 표시합니다.
...
반응형 스타일, 음성 자산 존재 여부까지 **327개**가 확인합니다.
